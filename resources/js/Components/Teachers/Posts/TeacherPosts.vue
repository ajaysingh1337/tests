<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="teacher_posts.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="post in teacher_posts.data" :key="post.id">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="teacher_posts.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditPostModal from "@/Components/Teachers/Posts/AddEditPostModal.vue";
import ViewPostModal from "@/Components/Teachers/Posts/ViewPostModal.vue";
import DeletePostModal from "@/Components/Teachers/Posts/DeletePostModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getTeacherPosts(loading_more)
     },
     clearModalData(){
        this.modal_post = null
        this.key++
     },
    getTeacherPosts(loading_more){
    axios.post(this.route('teachers.teacher_posts.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.teacher_posts.data = this.teacher_posts.data.concat(data.data);
        }else{
            this.teacher_posts.data = data.data;
        }
        this.teacher_posts.links = data.links
        this.teacher_posts.meta = data.meta
        this.fetching = false
    });
    },
  }
