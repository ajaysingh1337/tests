            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div v-for="(lang, secondIndex) in $page.props.translation_languages" :key="lang.id" class="tab-content mt-1" id="myTabContent">
                        <div :class="{'show active': secondIndex == 0}" class="tab-pane fade mb-2" :id="`nav-${lang.code}`" role="tabpanel" :aria-labelledby="`nav-${lang.code}-tab`">
                            ...
                            <div class="form-group mb-3">
                                <label class="mb-2 fs-4" for="name">{{__('name')}} ({{ lang.code }}) </label> <span class="text-danger">*</span>
                    ...
                    <div class="col-md-6">
                        <div v-if="form.link_type =='external'" class="form-group mb-3">
                            <label class="mb-2 fs-4" for="name">{{__('file link')}} </label> <span class="text-danger">*</span>
                        ...
                        <div v-if="form.link_type =='internal' && form.file_type == 'audio'" class="form-group mb-3 mt-2">
                            ...
                            <input id="teacher-audio" @change="onBroadcastFileChange($event,'audio')" type="file" :accept="getAcceptedFileTypes" />
                        ...
                        <div v-if="form.link_type =='internal' && form.file_type == 'video'" class="form-group mb-3 mt-2">
                            ...
                            <input id="teacher-video" @change="onBroadcastFileChange($event,'video')" type="file" :accept="getAcceptedFileTypes" />
                ...
                <!-- <div class="form-group mb-3">
                    ...
                    <!-- Cropped image previewer -->
                        <ImageCropperModal :show="showImportModal" id="broadcastImageCropModal" :image_url="image_url" @cropImage="cropImage" @close="cropClose" ></ImageCropperModal>
...
export default defineComponent({
    ...
    methods:{
        onFileChange(e) {
            console.log("CROP MODAL:", this.image_url, this.croppedImageSrc, this.showImportModal);
            this.image_url = null
        ...
        onBroadcastFileChange(e,type) {
            const file = e.target.files[0];
            if(!file.type.startsWith(this.form.file_type)) {
                this.$toast.error('Please select an audio file');
                return;
            } else {
                this.form[type] = file;
            }
        ...
        cropImage(image){
            ...
            // Reset file input more thoroughly to ensure change event fires on re-selection
            const fileInput = this.$refs.formImage;
            fileInput.value = '';
            fileInput.value = null;
            // Force a reflow to ensure the change is registered
    ...
    computed: {
        getAcceptedFileTypes() {
            ...
            if (this.form.file_type === 'audio') {
                acceptedFileTypes = 'audio/*';
            } else if (this.form.file_type === 'video') {
