<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="academy_archives.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="archive in academy_archives.data" :key="archive.id">
          ...
          <td class="align-middle"><span v-if="archive.is_active" class="badge bg-active rounded-pill px-3">{{ __('active') }}</span> <span class="badge bg-inactive rounded-pill px-3" v-else> {{ __('inactive') }} </span></td>
          <td class="align-middle">
            <div class="d-flex justify-content-center">
                <button type="button" class="bg-transparent fs-3 border-0 text-primary p-1 lh-1 me-2" data-bs-toggle="modal" @click="modal_archive = archive" data-bs-target="#viewArchiveModal">
                    <i class="bi bi-eye-fill"></i>
                  </button>
                  <button type="button" class="bg-transparent fs-3 border-0 p-1 lh-1 me-2"  style="color: #E2AE28;" data-bs-toggle="modal" @click="modal_archive = archive;" data-bs-target="#addEditArchiveModal">
                    <i class="bi bi-pencil-square"></i>
                  </button>
                  <button type="button" class="bg-transparent fs-3 border-0 p-1  lh-1" style="color: #FA6B6B ;" data-bs-toggle="modal" @click="modal_archive = archive" data-bs-target="#deleteArchiveModal">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="academy_archives.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditArchiveModal from "@/Components/Academies/Archives/AddEditArchiveModal.vue";
import ViewArchiveModal from "@/Components/Academies/Archives/ViewArchiveModal.vue";
import DeleteArchiveModal from "@/Components/Academies/Archives/DeleteArchiveModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getAcademyArchives(loading_more)
     },
     clearModalData(){
        this.modal_archive = null
        this.key++
     },
    getAcademyArchives(loading_more){
    axios.post(this.route('academies.academy_archives.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.academy_archives.data = this.academy_archives.data.concat(data.data);
        }else{
            this.academy_archives.data = data.data;
        }
        this.academy_archives.links = data.links
        this.academy_archives.meta = data.meta
        this.fetching = false
    });
    },
  }
