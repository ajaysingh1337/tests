<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="academy_services.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="service in academy_services.data" :key="service.id">
          ...
          <td class="align-middle">
            ...
            <button type="button" class="border-0  bg-transparent fs-3  text-primary lh-1 p-1 me-2" data-bs-toggle="modal" @click="modal_service = service" data-bs-target="#viewServiceModal">
              <i class="bi bi-eye-fill"></i>
            </button>
            <button type="button" class="border-0  bg-transparent  fs-3  lh-1 p-1 me-2" style="color: #E2AE28;" data-bs-toggle="modal" @click="modal_service = service;" data-bs-target="#addEditServiceModal">
              <i class="bi bi-pencil-square"></i>
            </button>
            <button type="button" class="border-0  bg-transparent  fs-3  lh-1 p-1" style="color: #FA6B6B;" data-bs-toggle="modal" @click="modal_service = service" data-bs-target="#deleteServiceModal">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="academy_services.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditServiceModal from "@/Components/Academies/Services/AddEditServiceModal.vue";
import ViewServiceModal from "@/Components/Academies/Services/ViewServiceModal.vue";
import DeleteServiceModal from "@/Components/Academies/Services/DeleteServiceModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getAcademyServices(loading_more)
     },
     clearModalData(){
        this.modal_service = null
        this.key++
     },
    getAcademyServices(loading_more){
    axios.post(this.route('academies.academy_services.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.academy_services.data = this.academy_services.data.concat(data.data);
        }else{
            this.academy_services.data = data.data;
        }
        this.academy_services.links = data.links
        this.academy_services.meta = data.meta
        this.fetching = false
    });
    },
  }
