<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="academy_certifications.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="certification in academy_certifications.data" :key="certification.id">
...
<!--            <img v-if="certification.image" :src="certification.image" width="75" height="75" :alt="certification.image" />-->
          ...
          <td class="align-middle"><span v-if="certification.is_active" class="badge bg-active rounded-pill px-3">{{ __('active') }}</span> <span class="badge bg-inactive rounded-pill px-3" v-else> {{ __('inactive') }}  </span></td>
          <td class="align-middle">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="academy_certifications.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditCertificationModal from "@/Components/Academies/Certifications/AddEditCertificationModal.vue";
import ViewCertificationModal from "@/Components/Academies/Certifications/ViewCertificationModal.vue";
import DeleteCertificationModal from "@/Components/Academies/Certifications/DeleteCertificationModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getAcademyCertifications(loading_more)
     },
     clearModalData(){
        this.modal_certification = null
        this.key++
     },
    getAcademyCertifications(loading_more){
    axios.post(this.route('academies.academy_certifications.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.academy_certifications.data = this.academy_certifications.data.concat(data.data);
        }else{
            this.academy_certifications.data = data.data;
        }
        this.academy_certifications.links = data.links
        this.academy_certifications.meta = data.meta
        this.fetching = false
    });
    },
  }
