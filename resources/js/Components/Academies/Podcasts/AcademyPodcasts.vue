<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="academy_podcasts.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="podcast in academy_podcasts.data" :key="podcast.id">
          ...
          <td class="align-middle"><span v-if="podcast.is_active" class="badge bg-active rounded-pill px-3">{{ __('active') }}</span> <span class="badge bg-inactive rounded-pill px-3" v-else> {{ __('inactive') }}  </span></td>
          <td class="align-middle">
            <div class="d-flex justify-content-center">
            <button type="button"  class="border-0  bg-transparent fs-3  text-primary lh-1 p-1 me-2" data-bs-toggle="modal" @click="modal_podcast = podcast" data-bs-target="#viewPodcastModal">
              <i class="bi bi-eye-fill"></i>
            </button>
            <button type="button" class="border-0  bg-transparent  fs-3  lh-1 p-1 me-2" style="color: #E2AE28;" data-bs-toggle="modal" @click="modal_podcast = podcast;" data-bs-target="#addEditPodcastModal">
              <i class="bi bi-pencil-square"></i>
            </button>
            <button type="button" class="border-0  bg-transparent  fs-3  lh-1 p-1" style="color: #FA6B6B;" data-bs-toggle="modal" @click="modal_podcast = podcast" data-bs-target="#deletePodcastModal">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="academy_podcasts.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditPodcastModal from "@/Components/Academies/Podcasts/AddEditPodcastModal.vue";
import ViewPodcastModal from "@/Components/Academies/Podcasts/ViewPodcastModal.vue";
import DeletePodcastModal from "@/Components/Academies/Podcasts/DeletePodcastModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getAcademyPodcasts(loading_more)
     },
     clearModalData(){
        this.modal_podcast = null
        this.key++
     },
    getAcademyPodcasts(loading_more){
    axios.post(this.route('academies.academy_podcasts.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.academy_podcasts.data = this.academy_podcasts.data.concat(data.data);
        }else{
            this.academy_podcasts.data = data.data;
        }
        this.academy_podcasts.links = data.links
        this.academy_podcasts.meta = data.meta
        this.fetching = false
    });
    },
  }
