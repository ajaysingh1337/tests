<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="academy_events.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="event in academy_events.data" :key="event.id">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="academy_events.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditEventModal from "@/Components/Academies/Events/AddEditEventModal.vue";
import ViewEventModal from "@/Components/Academies/Events/ViewEventModal.vue";
import DeleteEventModal from "@/Components/Academies/Events/DeleteEventModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getAcademyEvents(loading_more)
     },
     clearModalData(){
        this.modal_event = null
        this.key++
     },
    getAcademyEvents(loading_more){
    axios.post(this.route('academies.academy_events.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.academy_events.data = this.academy_events.data.concat(data.data);
        }else{
            this.academy_events.data = data.data;
        }
        this.academy_events.links = data.links
        this.academy_events.meta = data.meta
        this.fetching = false
    });
    },
  }
