<template>
    ...
    <div v-else>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="col-12">
                            <div class="row">
                                <div class="col-12">
                                    <div class="row p-4 bg-primary bg-opacity-10 rounded-5 mx-0">
                                        <h2 class="text-primary py-2 fw-bold fs-2">{{ __("Generate Appointment Slots")
                                            }}</h2>
                                        <hr />

                                        <label class="fs-5 fw-regular ms-1 mb-3" for="">{{ __("Toggle the button to designate holidays and working days") }}</label>

                                        <div
                                            class="col-12 switch d-flex gap-3 align-items-center mb-4 flex-column flex-md-row">
                                            <div v-for="(day, key) in weak_days" :key="key"
                                                class="form-check days-check py-2 rounded-3 p-0 bg-white form-switch shadow-sm">
                                                <label class="form-check-label fs-4 fw-bold mb-2"
                                                    :for="'flexSwitchCheckChecked' + key">{{ __(day.name) }}</label>
                                                <input class="form-check-input ms-0" type="checkbox" role="switch"
                                                    :value="day.value" :id="'flexSwitchCheckChecked' + key"
                                        ...
                                        <div class="col-lg-3 mb-3">
                                            <div class=" ps-0">
                                                <label class="fs-5 fw-bold mb-2" for="start_time">{{__("start time")}}</label>
                                                <input v-model="form.start_time" class="w-100 form-control  px-3"
                                                    :placeholder="__('Please Enter')" type="time"
                                                    v-on:input="enableButton()" />
                                            </div>
                                        </div>
                                        <div class="col-lg-3 mb-3">
                                            <div class=" ps-0">
                                                <label class="fs-5 fw-bold mb-2" for="end_time">{{__("end time")}}</label>
                                                <input v-model="form.end_time" class="w-100 form-control  px-3"
                                                    :placeholder="__('Please Enter')" type="time"
                                                    v-on:input="enableButton()" />
                                            </div>
                                        </div>
                                        <div class="col-lg-3 mb-3">
                                            <div class=" ps-0">
                                                <label class="fs-5 fw-bold mb-2" for="interval">{{__("interval")}}</label>
                                                <input v-model="form.interval" class="w-100 form-control  px-3"
                                                    :placeholder="__('Please Enter')" type="integer"
                                                    v-on:input="enableButton()" />
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-lg-3">
                                            <div class=" ps-0">
                                                <label class="fs-5 fw-bold mb-2" for="fee">{{__("fee")}}</label>
                                                <div class="input-group mb-3">
                                                    <span class="input-group-text"
                                                        id="basic-addon1">{{ getDefaultCurrencySymbol() }}</span>
                                                    <input v-model="form.fee" class="form-control px-3"
                                                        :placeholder="__('Please Enter')" type="text"
                                                        v-on:input="enableButton()" v-on:keypress="inputNumbersOnly()"
                                        ...
                                        <div class="col-md-4" v-if="isCommissionEnabled()">
                                            ...
                                            <div class="input-group d-flex align-items-center mb-3">
                                                <input class="form-control px-3"
                                                    :placeholder="getDisplayAmount(feeAfterCommission)" type="text"
                                                    disabled />
                                        ...
                                        <div class="col-12">
                                            <p v-if="isCommissionEnabled()"><b>Note:</b>
                                                The administrative has determined that the commission for educational services
                                                is <span
                                                    v-if="commission && commission.commission_type == 'fixed_rate'">{{getDefaultCurrencySymbol() }} {{ commission.rate }}</span> <span
                                                    v-if="commission && commission.commission_type == 'percentage'"> {{commission.rate }}%</span> of the final amount, which is equivalent
                                                to {{ getDisplayAmount(feeAfterCommission) }}.
                                            </p>
                                    ...
                                    <div class="row px-0">
                                        <div class="col-12 px-0 text-end">
                                            <button type="button" @click="generateTimeSlots"
                                                class="submit mt-4 btn btn-primary px-5" :disabled="disableButton">
                                                {{ __("generate slots") }}
                                            </button>
                                        </div>
                            ...
                            <div v-if="Object.keys(form.generated_slots).length > 0"
                                class="col-12 cat-card bg-white p-4 time-list mt-4 rounded-4">
                                <div>
                                    <div
                                        class="d-flex align-items-center justify-content-between pb-2 mb-4 border-bottom border-primary">
                                        <div>
                                            <span class="fw-bold me-3">{{ __('You have successfully generated slots')
                                                }}</span>
                                        </div>
                                    ...
                                    <div class="accordion border-0 p-0" id="accordionExample">
                                        <div class="accordion-item  border-0  rounded-4 mb-3"
                                            v-for="(day, dayIndex) in weak_days" :key="dayIndex">
                                            <div class="accordion-header rounded-4 shadow-none d-flex align-items-center justify-content-between w-100 py-2 px-3"
                                                style="background-color: #E7F3FA;"
                                                :class="{ 'bg-danger bg-opacity-25': dayIndex == 0 }"
                                                :id="'headingOne' + dayIndex">
                                                <div class="accordion-button bg-transparent shadow-none p-1 collapsed"
                                                    type="button" data-bs-toggle="collapse"
                                                    :data-bs-target="'#collapseOne' + dayIndex" aria-expanded="true"
                                            ...
                                            <div :id="'collapseOne' + dayIndex" class="accordion-collapse collapse show"
                                                :aria-labelledby="'headingOne' + dayIndex">
                                                <div class="accordion-body p-2 time-slots-list"
                                                    v-if="Object.keys(form.generated_slots).length > 0">
                                                    <div v-if="form.generated_slots[day.value]"
                                                        class="position-relative">
                                                        <ul class="list-unstyled d-flex flex-wrap">
                                                            <li class="mb-2 p-2 m-1 shadow-sm" v-for="(slot, index) in form.generated_slots[
        day.value
    ]" :key="index" :class="slot.is_active
            ? ''
        ...
        " @click="changeSlotStatus(index, day.value)">
                                                    ...
                                                    <div class="position-relative py-5 d-flex align-items-center justify-content-center h-100"
                                                        v-else>
                                                        <h3>
                                                            <i class="bi bi-x-octagon-fill fs-2 text-primary"></i>
                                                            {{ __("holiday") }}
                                                        </h3>
                                                    </div>
                                                </div>
                                ...
                                <div class="row align-items-center" v-if="Object.keys(form.generated_slots).length > 0
        ">
        ...
        <div v-else class="row mt-4 mx-0 bg-white cat-card p-3 time-list rounded-4">
        <div>
            <div class="d-flex align-items-center justify-content-between pb-3 mb-4 border-bottom border-primary">
                <div>
                    <span class="fw-bold me-3">{{ __("Your Generated Slots") }}</span>
                </div>
                <div class="d-flex align-items-center">
                    <div class="me-0 me-md-3">
                        <span class="fw-bold me-2">{{ __("interval") }}:</span> <span>
            ...
            <div class="accordion border-0 p-0" id="accordionExample">
                <div class="accordion-item border-0  rounded-4 mb-3" v-for="(day, dayIndex) in weak_days"
                    :key="dayIndex">
                    <div class="accordion-header rounded-4 shadow-none d-flex align-items-center justify-content-between w-100 py-2 px-3"
                        style="background-color: #E7F5FD"
                        :class="schedules[day.value] ? 'bg-warning' : 'bg-danger bg-opacity-25'"
                        :id="'headingOne' + dayIndex">
                        <div class="accordion-button bg-transparent shadow-none p-1 collapsed" type="button"
                            data-bs-toggle="collapse" :data-bs-target="'#collapseOne' + dayIndex" aria-expanded="true"
                            :aria-controls="'collapseOne' + dayIndex">

                            <div class="fs-4 fw-bold mb-0 ms-4">{{ day.name }}</div>

                        </div>
                        <button v-if="schedules[day.value]" type="button"
                            class="btn btn-transparent p-0 border-0 shadow-none" data-bs-toggle="modal"
                            :data-bs-target="'#deleteScheduleModal_' + dayIndex">
                            <i class="bi bi-trash fs-3 px-1 fw-bolder text-danger"></i>
                        </button>
                        <button v-else type="button" class="btn bg-primary p-0 border-0 shadow-none"
                            @click="() => (addForm.day = day.value)" data-bs-toggle="modal"
                        ...
                        <div class="accordion-body p-2 position-relative time-slots-list">
                            <div v-if="schedules[day.value]" class="position-relative">
                                <ul class="list-unstyled d-flex flex-wrap mb-0">
                                    <li class="mb-2 p-2 m-1 shadow-sm" v-for="(slot, index) in schedules[day.value].schedule_slots" :key="index">
                                        <span>{{ slot.start_time }} - {{ slot.end_time }}</span>
                                        <!-- <i
                                  class="bi bi-arrow-left-right text-primary"
                            ...
                            <div class="py-2 d-flex align-items-center justify-content-center h-100" v-else>
                    ...
                    <!-- Delete Modal -->
                    <Modal maxWidth="lg" :id="'deleteScheduleModal_' + dayIndex"
                        :aria-labelledby="'deleteScheduleModalLabel' + dayIndex">
                        <form @submit.prevent="deleteSelectDaySlots(day.value,'deleteScheduleModalClose_' + dayIndex)">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" :id="'deleteScheduleModalLabel' + dayIndex">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="submit" :disabled="deleteForm.processing" class="btn btn-primary">
                                        {{ __("yes") }}
                                    </button>
                                </div>
                            </div>
                    ...
                    >
                      <div class="modal-dialog">
                        <form
                          @submit.prevent="
                            deleteSelectDaySlots(day.value, 'deleteScheduleModalClose_' + dayIndex)">
                          <div class="modal-content">
                            <div class="modal-header">
                    ...
                    <!-- Add Modal -->
                    <Modal maxWidth="xl" :id="'addScheduleModal_' + dayIndex"
                        :aria-labelledby="'addSlotsModelLabel' + dayIndex">
                        <form @submit.prevent="addSelectDaySlots(day.value,'addScheduleModalClose_' + dayIndex)">
                            <div class="modal-content">
                                <div class="modal-header">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="row px-3">
                                        <div class="col-4">
                                            <div class="">
                                                <label class="fs-5 fw-bold mb-2" for="start_time">{{__("start time")}}</label>
                                                <input v-model="addForm.start_time"
                                        ...
                                        <div class="col-4">
                                            <div class="">
                                                <label class="fs-5 fw-bold mb-2" for="end_time">{{ __("end time")}}</label>
                                                <input v-model="addForm.end_time" class="w-100 form-control px-3"
                                                    :placeholder="__('Please Enter')" type="time" />
                                            </div>
                                        </div>
                                        <div class="col-4">
                                            <div class="">
                                                <label class="fs-5 fw-bold mb-2" for="interval">{{ __("interval")}}</label>
                                                <input v-model="addForm.interval" class="w-100 form-control px-3"
                                                    :placeholder="__('Please Enter')" type="integer" />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row px-3">
                                        <div class="col-12">
                                            <button type="button" @click="generateTimeSlotsForAdd"
                                    ...
                                    <div v-if="addForm.generated_slots.length > 0">
                                        <div class="row border shadow-sm mt-3 rounded-5 mx-3">
                                            ...
                                            <div class="col-md-12 mt-3 time-slots-list"
                                                v-if="addForm.generated_slots.length > 0">
                                                <ul class="list-unstyled d-flex flex-wrap">
                                                    <li class="mb-2 p-2 m-1 rounded-5 border border-primary" v-for="(slot, slotIndex) in addForm.generated_slots" :key="slotIndex">
                                                        <span>{{ slot.start_time }}
                                                            <i class="bi bi-arrow-left-right text-primary"
                                                                :class="slot.is_active ? '' : 'text-muted'"></i>
                                                            {{ slot.end_time }}</span>
                                                    </li>
                                                </ul>
                                            </div>
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="submit" :disabled="addForm.processing" class="btn btn-primary">
                                        {{ __("submit") }}
                                    </button>
                                </div>
                            </div>
                    ...
                    >
                      <div class="modal-dialog modal-xl">
                        <form
                          @submit.prevent="
                            addSelectDaySlots(day.value,'addScheduleModalClose_' + dayIndex)">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title" id="exampleModalLabel">
                                {{ __("Add Schedule Model") }}
                                    ...
                                    <ul class="list-unstyled d-flex flex-wrap">
                                      <li
                                       class="mb-2 p-2 m-1 rounded-5 border border-primary"
                                        v-for="(slot, slotIndex) in addForm.generated_slots" :key="slotIndex">
                                        <span
                                          >{{ slot.start_time }}
                                          <i
                                            class="bi bi-arrow-left-right text-primary"
...
export default defineComponent({
    ...
    data() {
        return {
            weak_days: {
                sunday: { name: "Sunday", value: "sunday", isSelected: false },
                monday: { name: "Monday", value: "monday", isSelected: false },
                tuesday: { name: "Tuesday", value: "tuesday", isSelected: false },
                wednesday: { name: "Wednesday", value: "wednesday", isSelected: false },
                thursday: { name: "Thursday", value: "thursday", isSelected: false },
                friday: { name: "Friday", value: "friday", isSelected: false },
                saturday: { name: "Saturday", value: "saturday", isSelected: false },
            },
    ...
    methods: {
        updateSelectedDays() {
            this.form.selected_days = Object.keys(this.weak_days).filter(day => this.weak_days[day].isSelected);
        },
        getAppointmentschedules() {
            this.fetching = true;
            axios
                .get(
                    this.route("academy.getApiAppointmentSchedules", {
        ...
        getAppointmentCommission() {
            ...
            axios
                .get(
                    this.route("academy.getApiAppointmentCommission", {
                        appointment_type_id: this.form.appointment_type_id,
                    })
                )
                .then((res) => {
        ...
        generateTimeSlots() {
            ...
            while (currentTime < end) {
                ...
                let startTime = currentTime.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
                let endTime = currentTime.setTime(currentTime.getTime() + intervalMs) <= end.getTime()
                    ? currentTime.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
                    : end.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
                let is_active = true;
                slots.push({ start_time: startTime, end_time: endTime, is_active: is_active });
            }

            Object.keys(this.weak_days).forEach(dayKey => {
                if (this.weak_days[dayKey].isSelected) {
                    this.form.generated_slots[dayKey] = slots;
                }
            });
        },
        ...
        submit() {
            this.form.post(this.route("academy.save_appointment_schedules"), {
                onSuccess: () => {
        ...
        deleteSelectDaySlots(day_value, model_id) {
            ...
            this.deleteForm.post(
                this.route("academy.delete_appointment_slots"),
                {
                    onSuccess: () => {
        ...
        addSelectDaySlots(day_value, model_id) {
            ...
            this.addForm.post(
                this.route("academy.add_new_appointment_schedules"),
                {
                    onSuccess: () => {
                        document.getElementById(model_id).click();
                        this.addForm.day = "";
