<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="academy_posts.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="post in academy_posts.data" :key="post.id">
          ...
          <td class="align-middle"><span v-if="post.is_active" class="badge bg-active rounded-pill px-3">{{ __('active') }}</span> <span class="badge bg-inactive rounded-pill px-3" v-else> {{ __('inactive') }} </span></td>
          <td class="align-middle">
             <div class="d-flex justify-content-center">
            <button type="button" class="border-0 bg-transparent fs-3 text-primary p-1 lh-1 me-2" data-bs-toggle="modal" @click="modal_post = post" data-bs-target="#viewPostModal">
              <i class="bi bi-eye-fill"></i>
            </button>
            <button type="button" class="bg-transparent border-0 fs-3 p-1 lh-1 me-2"  style="color: #E2AE28;" data-bs-toggle="modal" @click="modal_post = post;" data-bs-target="#addEditPostModal">
              <i class="bi bi-pencil-square"></i>
            </button>
            <button type="button" class="bg-transparent border-0 fs-3  p-1 lh-1" style="color: #FA6B6B ;" data-bs-toggle="modal" @click="modal_post = post" data-bs-target="#deletePostModal">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="academy_posts.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditPostModal from "@/Components/Academies/Posts/AddEditPostModal.vue";
import ViewPostModal from "@/Components/Academies/Posts/ViewPostModal.vue";
import DeletePostModal from "@/Components/Academies/Posts/DeletePostModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getAcademyPosts(loading_more)
     },
     clearModalData(){
        this.modal_post = null
        this.key++
     },
    getAcademyPosts(loading_more){
    axios.post(this.route('academies.academy_posts.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.academy_posts.data = this.academy_posts.data.concat(data.data);
        }else{
            this.academy_posts.data = data.data;
        }
        this.academy_posts.links = data.links
        this.academy_posts.meta = data.meta
        this.fetching = false
    });
    },
  }
