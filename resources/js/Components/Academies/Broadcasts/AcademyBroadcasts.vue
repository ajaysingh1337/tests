<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="academy_broadcasts.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="broadcast in academy_broadcasts.data" :key="broadcast.id">
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="academy_broadcasts.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditBroadcastModal from "@/Components/Academies/Broadcasts/AddEditBroadcastModal.vue";
import ViewBroadcastModal from "@/Components/Academies/Broadcasts/ViewBroadcastModal.vue";
import DeleteBroadcastModal from "@/Components/Academies/Broadcasts/DeleteBroadcastModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getAcademyBroadcasts(loading_more)
     },
     clearModalData(){
        this.modal_broadcast = null
        this.key++
     },
    getAcademyBroadcasts(loading_more){
    axios.post(this.route('academies.academy_broadcasts.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.academy_broadcasts.data = this.academy_broadcasts.data.concat(data.data);
        }else{
            this.academy_broadcasts.data = data.data;
        }
        this.academy_broadcasts.links = data.links
        this.academy_broadcasts.meta = data.meta
        this.fetching = false
    });
    },
  }
