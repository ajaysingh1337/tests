...
import GoLiveOptions from "./GoLiveOptions";
...
class CallActions extends React.Component {
  ...
  attachSocketListeners() {
    console.log('actions attachSocketListeners', this.context.socket);
  ...
  removeSocketListeners() {
    ...
    this.context.socket.removeAllListeners(
          "new_message_" + this.call.type + "_" + this.props.record_id
        );
    }
  ...
  endCall() {
    ...
    console.log('this.call',this.call,this.current_user);
    const callData={
      id:this.call.app_data.id,
      type:this.current_user.role,
      user_id:this.current_user.id
    }
    axios.post(`/call/end/${callData.id}`, callData);

    if (this.props.call_options.urls.call_ended) {
      axios.post(this.props.call_options.urls.call_ended, callData);
    }
  ...
  endCallFromWeb() {
    ...
    //console.log('endCallFromWeb',window);
  ...
  userEventDetail(event_name) {
    return;
    console.log("share screen", common.getCurrentUser(), event_name);

    if (common.getCurrentUser()) {
      var platform;
      var env_name = process.env.REACT_APP_ENVOIRNMENT_KEY;
      var app_version = process.env.REACT_APP_MIZDAH_VERSION;
      if (env_name == "web") {
        platform = "web";
      } else {
        platform = process.env.REACT_APP_ENVOIRNMENT_PLATFORM;
      }

      axios.get("users/user_event_detail", {
        params: {
          meeting_id: this.call.id,
  ...
  goLive() {
    axios
      .post("https://streaming.mizdah.com:3000/go_live", {
        // axios.post("http://localhost:5000/go_live", {
      ...
      .then((response) => {
        ...
        if (response.data.code == 200) {
          this.setState({ go_live_page_id: response.data.page_id });
          this.setState({ live_stream_id: response.data.live_stream_id });
          console.log("close page", this.state.go_live_page_id);
        }
      })
      .catch((error) => {
        console.log(error);
  ...
  closeGoLive() {
    axios
      .post("https://streaming.mizdah.com:3000/close_go_live_page", {
        // axios.post("http://localhost:5000/close_go_live_page", {
        go_live_page_id: this.state.go_live_page_id,
        ignore_encryption: true,
      })
      .then((response) => {
        console.log(response);
        if (response.data.code == 200) {
          this.setState({ go_live_page_id: "" });
          this.setState({ live_stream_id: "" });
        }
      })
      .catch((error) => {
        console.log(error);
