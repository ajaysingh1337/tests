...
import RtcClient from "webrtc-client/src/rtc_client";
...
export default class CallLibrary extends React.Component {
	...
	constructor(props) {
		super(props);
        console.log('CallLibrary props',this.props);
        ...
        this.room = {
            ...
            external_id: this.props.call.app_data.meeting_id,
    ...
    async initiateLibrary() {
        ...
        rtc_client.room_type = 'p2p-sfu';
        rtc_client.user = this.props.current_user;
        rtc_client.socket = this.context.ms_socket;
        rtc_client.socket_io = io;
        rtc_client.server_url = import.meta.env.VITE_CALL_SERVER_URL;

        let ice_servers = [
                        {
                            url: 'stun:stun.ogoul.com:3478',
                            urls: 'stun:stun.ogoul.com:3478',
                        },
                        {
                            urls: "stun:stun.l.google.com:19302"
                        },
                        {
                            url: 'turn:turn.ogoul.com:3478',
                            username: 'ogoul',
                            urls: 'turn:turn.ogoul.com:3478',
                            credential: 'Ogoul@ST!$',
                        }
        ];

        try {
            var response = await axios.get(`${rtc_client.server_url}/get_ice_servers`, { timeout: 5000 });
            var ice_servers_from_server = response.data && response.data.iceServers;
            if(ice_servers_from_server) {
                //ice_servers = [...ice_servers,ice_servers_from_server];
                ice_servers = ice_servers_from_server;
            }
