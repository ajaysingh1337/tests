...
export default class CallTimeMaster extends React.Component {
  ...
  attachSocketListeners() {
    ...
    this.context.socket.once(this.socket_events.call_started, (data) => {
      var call_id = data.meeting_id;

      if (call_id != this.call.id) return;

      //auto joining, was waiting for host to start meeting
      if (!this.state.call_allowed) {
        //setting current utc time as the call started now
        this.timing.start_time = new Date().toUTCString();
        this.call_started = true;

        this.allowCall();
        this.decideToShowInfo();
      }
    });

    this.context.socket.on("call__extend_time" + this.call.id + this.call.type, (data) => {
        var minutes_to_extend = Number(data.extend_time);
        this.duration_minutes = Number(this.duration_minutes) + minutes_to_extend;
        this.timing.duration_minutes = this.duration_minutes;

        var end_time_local_obj = moment(this.state.start_time_local_obj).add(
          this.duration_minutes,
          "minutes"
        );
        var end_time_local = datetime.formatDateToDb(end_time_local_obj, true);
