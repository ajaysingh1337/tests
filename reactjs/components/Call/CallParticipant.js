...
class CallParticipant extends React.Component {
    ...
    async makeHost() {
        ...
        axios.post("meeting_users/make_host", {
            meeting_id: window.call_helpers.call.id,
            user_id: this.user.id,
            guest_user_name: this.user.name
        });
    }
    ...
    async removeHost() {
        ...
        axios.post("meeting_users/remove_host", {
            meeting_id: window.call_helpers.call.id,
            user_id: this.user.id,
            guest_user_name: this.user.name
        });
    ...
    async moveUserToWaitingArea() {
        ...
        axios.post("meeting_users/move_to_waiting_area", {
            id: window.call_helpers.call.id,
            user_id: this.user.id,
    ...
    async blockUser(user) {
        ...
        axios.post("meeting_users/block_user", {
            id: window.call_helpers.call.id,
            user_id: this.user.id,
            guest_user_name: this.user.name
        });
    ...
    async removeUser(remove_user_options = {}) {
        remove_user_options.target_user_id = this.user.id;
    
        var result = await this.props.rtc_client.removeUser(remove_user_options);
        if(!result) return;
    
        if (!remove_user_options.is_blocked && !remove_user_options.is_waiting) {
            axios.post("meeting_users/remove_from_session", {
                id: window.call_helpers.call.id,
                user_id: this.user.id,
                guest_user_name: this.user.name
            });
        ...
        this.setState({ remove_underway: true });
    
        // just a precautionary step
        setTimeout(() => {
            this.setState({ remove_underway: false });
        }, 5000);
