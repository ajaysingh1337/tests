    ...
    public static function prepareData($order, $gateway)
    {
        if ($order->type == 'wallet') {
            ...
            if ($order->appointment->teacher_id) {
                ...
                $success_route = '/teacher/profile/' . $user_name . '/book_appointment?type=' . $appointment_type . '&paymentSuccess=true&appointmentId=' . $appointment_id;
            } elseif ($order->appointment->academy_id) {
                $user_name = $order->appointment->academy->user_name;
                $success_route = '/academy/profile/' . $user_name . '/book_appointment?type=' . $appointment_type . '&paymentSuccess=true&appointmentId=' . $appointment_id;
            }
        }
        ...
        $val['student_first_name'] = str_replace('.', '', optional($order->user)->firstname) ?? "John";
        $val['student_last_name'] = optional($order->user)->lastname ?? "Doe";
        $val['student_address'] = optional($order->user)->address ?? "10 Downing Street";
        $val['student_city'] = optional($order->user)->city ?? "London";
        $val['student_zip_code'] = optional($order->user)->zip_code ?? "121165";
        $val['student_country'] = "GB";
        $val['student_phone'] = optional($order->user)->phone ?? "+79000000000";
        $val['student_email'] = optional($order->user)->email ?? "johndoe@mail.com";

        $val['student_ip_address'] = request()->ip();
