...
// Students
$student = $api->student->create(array('name' => 'Razorpay User', 'email' => 'student@razorpay.com')); // Creates student
$student = $api->student->fetch($studentId); // Returns a particular student
$student = $api->student->edit(array('name' => 'Razorpay User', 'email' => 'student@razorpay.com')); // Edits student

// Tokens
$token  = $api->student->token()->fetch($tokenId); // Returns a particular token
$tokens = $api->student->token()->all($options); // Returns array of token objects
$api->student->token()->delete($tokenId); // Deletes a token
...
$link  = $api->invoice->create(arary('type' => 'link', 'amount' => 500, 'description' => 'For XYZ purpose', 'student' => array('email' => 'test@test.test')));
$link->cancel();
$link->notifyBy('sms');
...
$plans         = $api->plan->all();
$subscription  = $api->subscription->create(array('plan_id' => 'plan_7wAosPWtrkhqZw', 'student_notify' => 1, 'total_count' => 6, 'start_at' => 1495995837, 'addons' => array(array('item' => array('name' => 'Delivery charges', 'amount' => 30000, 'currency' => 'INR')))));
